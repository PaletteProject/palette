...
export type LocalGrade = {
  ...
  export function getStoredGrades(
    ...
    try {
      const storageKey = getStorageKey(activeCourseId, activeAssignmentId, rubricId);
  ...
  export function updateGrade(
    ...
    const storageKey = getStorageKey(activeCourseId, activeAssignmentId, rubricId);
    const grades = JSON.parse(localStorage.getItem(storageKey) || "[]");
  
    const index = grades.findIndex((g: any) => g.userId === userId);
    if (index !== -1) {
   ...
   */
  export function clearGrades(activeCourseId: string, activeAssignmentId: string, rubricId: string) {
    try {
      const storageKey = getStorageKey(activeCourseId, activeAssignmentId, rubricId);
